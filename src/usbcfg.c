...
#define CDC_IF_DESC_SET(comIfNum, datIfNum, comInEp, datOutEp, datInEp)        \
    ...
    USB_DESC_INTERFACE(                                                        \
        ...
        /* bDataInterface.          */   /* Abstract Control                   \
                                            Management Functional              \
                                            Descriptor.*/                      \
        USB_DESC_BYTE(4),                /* bFunctionLength.         */        \
        USB_DESC_BYTE(CDC_CS_INTERFACE), /* bDescriptorType.         */        \
        USB_DESC_BYTE(CDC_ABSTRACT_CONTROL_MANAGEMENT), USB_DESC_BYTE(0x02),   \
        /* bmCapabilities.          */   /* Union Functional                   \
                                            Descriptor.*/                      \
        USB_DESC_BYTE(5),                /* bFunctionLength.         */        \
        USB_DESC_BYTE(CDC_CS_INTERFACE), /* bDescriptorType.         */        \
        ...
        USB_DESC_BYTE(datIfNum),                                               \
        /* bSlaveInterface.         */ /* Endpoint, Interrupt                  \
                                          IN.*/                                \
        USB_DESC_ENDPOINT(comInEp,                                             \
                          USB_EP_MODE_TYPE_INTR,      /* bmAttributes.      */ \
                          USB_INTERRUPT_REQUEST_SIZE, /* wMaxPacketSize. */    \
                          0x01), /* bInterval.               */                \
                                                                               \
        /* CDC Data Interface Descriptor.*/                                    \
        USB_DESC_INTERFACE(datIfNum, /* bInterfaceNumber.        */            \
                           0x00,     /* bAlternateSetting.       */            \
                           0x02,     /* bNumEndpoints.           */            \
 ...
 */
static const USBEndpointConfig ep2config = {USB_EP_MODE_TYPE_BULK,
                                            NULL,
                                            sduDataTransmitted,
                                            sduDataReceived,
                                            USB_DATA_SIZE,
 ...
 */
static const USBEndpointConfig ep3config = {USB_EP_MODE_TYPE_INTR,
                                            NULL,
                                            sduInterruptTransmitted,
                                            NULL,
                                            USB_INTERRUPT_REQUEST_SIZE,
 ...
 */
static const USBEndpointConfig ep4config = {USB_EP_MODE_TYPE_BULK,
                                            NULL,
                                            sduDataTransmitted,
                                            sduDataReceived,
                                            USB_DATA_SIZE,
...
const USBConfig usbcfg = {
    usb_event, get_descriptor, requests_hook, sof_handler};

/*
 * Serial over USB driver configuration 1.
 */
const SerialUSBConfig serusbcfg1 = {&USBD1,
